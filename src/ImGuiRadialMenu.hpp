...
bool IsPopupOpen(const char* pName)
{
	...
	return g.OpenPopupStack.Size > g.CurrentPopupStack.Size && g.OpenPopupStack[g.CurrentPopupStack.Size].PopupId == iID;
}
...
void EndPiePopup()
{
	...
	for (int iIndex = 0; iIndex <= s_oPieMenuContext.m_iMaxIndex; ++iIndex)
	{
		...
		for (int item_n = 0; item_n < oPieMenu.m_iCurrentIndex; item_n++)
		{
			...
			ImU32 iColor = hovered ? ImColor(200, 200, 150) : ImColor(70, 70, 70);
